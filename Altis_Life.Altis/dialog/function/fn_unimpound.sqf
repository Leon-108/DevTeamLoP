...
if( life_garage_sp isEqualType []) then {
	if (life_HC_isActive) then {
        [_vid,_pid,(life_garage_sp select 0),_unit,_price,(life_garage_sp select 1),_spawntext,_ctype,_type] remoteExec ["HC_fnc_spawnVehicle",HC_Life];
    } else {
        [_vid,_pid,(life_garage_sp select 0),_unit,_price,(life_garage_sp select 1),_spawntext,_ctype,_type] remoteExec ["TON_fnc_spawnVehicle",RSERV];
    };
} else {
	_markerDir = if !(life_garage_sp isEqualTo "taxi") then {markerDir life_garage_sp} else {0};
	if(life_garage_sp in ["medic_spawn_1","medic_spawn_2","medic_spawn_3"]) then {
		if (life_HC_isActive) then {
            [_vid,_pid,life_garage_sp,_unit,_price,0,_spawntext,_ctype,_type] remoteExec ["HC_fnc_spawnVehicle",HC_Life];
        } else {
            [_vid,_pid,life_garage_sp,_unit,_price,0,_spawntext,_ctype,_type] remoteExec ["TON_fnc_spawnVehicle",RSERV];
        };
	} else {
		...
		if (life_garage_sp isEqualTo "taxi") then {
			...
			if (count _pos isEqualTo 3) then {
				...
				if (life_HC_isActive) then {
					[_vid,_pid,_pos,_unit,_price,_markerDir,_spawntext,_ctype,_type] remoteExec ["HC_fnc_spawnVehicle",HC_Life];
				} else {
					[_vid,_pid,_pos,_unit,_price,_markerDir,_spawntext,_ctype,_type] remoteExec ["TON_fnc_spawnVehicle",RSERV];
				};
