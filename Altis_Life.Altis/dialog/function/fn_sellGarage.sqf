...
if(!_action) exitWith {["Verkaufen abgebrochen","RED",10] spawn life_fnc_notification_system;};
...
if (life_HC_isActive) then {
    ...
    [_vid,_pid,_price,player,life_garage_type] remoteExecCall ["TON_fnc_vehicleDelete",RSERV];
};
...
if (LIFE_SETTINGS(getNumber,"player_advancedLog") isEqualTo 1) then {
    if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
        advanced_log = format [localize "STR_DL_AL_soldVehicle_BEF",_vehicleLife,[_price] call life_fnc_numberText,[BANK] call life_fnc_numberText,[CASH] call life_fnc_numberText];
    } else {
        advanced_log = format [localize "STR_DL_AL_soldVehicle",profileName,(getPlayerUID player),_vehicleLife,[_price] call life_fnc_numberText,[BANK] call life_fnc_numberText,[CASH] call life_fnc_numberText];
    };
