		...
		if((_chance > (_x select 1) && (_chance <= (_x select 2)))) exitWith {
			for "_a" from 0 to (count _newGivenItems) step 1 do {
				if((_newGivenItems select _a select 0) isEqualTo _itemtemp) exitWith {
					...
					_val =_val +1;
					_newGivenItems set [_a,[_itemtemp,_val]];
					_itemAdded pushBackUnique _itemtemp;
				};
			};
			if !(_itemtemp in _itemAdded) exitWith {
				_newGivenItems set [(count _newGivenItems),[_itemtemp,1]];
			};
        };
    } forEach _newItem;
};
...
if(_hasLicense) then
{
    ...
    if (player distance _vendor > 10) exitWith {
		[localize "STR_Process_Stay","YELLOW",5] spawn life_fnc_notification_system;
		"progressBar" cutText ["","PLAIN"];
		life_is_processing = false;
		life_action_inUse = false;
	};
