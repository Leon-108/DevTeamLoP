...
if((time - life_action_delay) < 2) exitWith {[localize "STR_NOTF_ActionDelay","RED",10] spawn life_fnc_notification_system; INUSE(_this);};
...
if (playerSide isEqualTo west && _illegal isEqualTo 1) exitWith {
	[format[localize "STR_NOTF_PickedEvidence",_itemName,([round(ITEM_SELLPRICE(_itemInfo select 0))] call life_fnc_numberText)],"GREEN",5] spawn life_fnc_notification_system;
	BANK = BANK + round(ITEM_SELLPRICE(_itemInfo select 0));
	deleteVehicle _this;
...
if(_diff <= 0) exitWith { [localize "STR_NOTF_InvFull","RED",10] spawn life_fnc_notification_system; INUSE(_this);};
...
if (!(_diff isEqualTo (_itemInfo select 1))) then {
    if ([true,(_itemInfo select 0),_diff] call life_fnc_handleInv) then {
		...
		[format[localize "STR_NOTF_Picked",_diff,_itemName],"GREEN",5] spawn life_fnc_notification_system;
		INUSE(_this);
    } else {
        INUSE(_this);
	};
