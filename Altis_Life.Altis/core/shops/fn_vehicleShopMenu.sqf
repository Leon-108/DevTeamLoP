...
if(_sideCheck != sideUnknown && {playerSide != _sideCheck}) exitWith {
	[localize "STR_Shop_Veh_NotAllowed","RED",10] spawn life_fnc_notification_system;
};
private _conditions = M_CONFIG(getText,"CarShops",_shop,"conditions");
if !([_conditions] call life_fnc_levelCheck) exitWith {[localize "STR_Shop_Veh_NotAllowed","RED",10] spawn life_fnc_notification_system;};
