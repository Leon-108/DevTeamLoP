...
if(_shooter isKindOf "Man" && alive player) then
{
	if(!life_istazed) then
	{
		...
		[[player,"AinjPfalMstpSnonWrflDnon_carried_still"],"life_fnc_animSync",true,false] spawn life_fnc_MP;
		if (side _shooter isEqualTo west) then {
			...
			[[0,"STR_NOTF_CIVTazed",true,[profileName, _shooter getVariable["realname",name _shooter]]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		};
