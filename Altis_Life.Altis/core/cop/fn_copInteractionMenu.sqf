...
if(isNull _curTarget && (_closestMan isEqualTo "")) exitWith {closeDialog 0;}; //Bad target

if(!isPlayer _closestMan && side _closestMan isEqualTo civilian) exitWith {closeDialog 0;}; //Bad side check?
if !(_closestMan isEqualTo "") exitWith {
	...
	if((_closestMan getVariable["Escorting",false])) then {
		...
		_Btn4 buttonSetAction "[life_pInact_curTarget] call life_fnc_stopEscorting; [life_pInact_curTarget,life_pInact_curTarget] call life_fnc_copInteractionMenu;";
	} else {
	...
	if(!((player distance (getMarkerPos "police_hq_1") < 30) OR  (player distance (getMarkerPos "police_hq_2") < 30) OR (player distance (getMarkerPos "cop_spawn_3") < 30) OR (player distance (getMarkerPos "cop_spawn_5") < 30) OR (player distance (getMarkerPos "cop_spawn_7") < 30) OR (player distance (getMarkerPos "jail1_marker") < 100) OR (player distance (getMarkerPos "cop_spawn_4") < 40) OR (life_pInact_curTarget in life_known_Players))) then 
	{
...
if(_curTarget isKindOf "House_F" && cursorTarget distance player < 2.5) exitWith {
