...
/*
	life_nextrobbankbranch
	TON_fnc_robbankbranch
	life_firstrobBankBranch
*/
private["_robber","_shop","_kassa","_ui","_progress","_pgText","_cP","_rip","_pos", "_robdelay", "_delay","_cops","_onWanted"];
...
if (life_firstrobBankBranch) exitWith {
...
if (servertime < life_nextrobbankbranch) exitWith {
	//h/intSilent format ["Es wurde gerade schon eine Bank überfallen, komm in %1 Minuten wieder vorbei!", [(life_nextrob - servertime),"MM:SS"] call BIS_fnc_secondsToString]
	[format ["Es wurde gerade schon eine Bank überfallen, komm in %1 Minuten wieder vorbei!", [(life_nextrob - servertime),"MM:SS"] call BIS_fnc_secondsToString],"RED",10] spawn life_fnc_notification_system;
};
_cops = (west countSide playableUnits);
if (_cops < 10) then {
	_robdelay = 2700; // 2700 45 Minuten Zeit die zwischen zwei Überfällen vergehen muss.
} else {
	_robdelay = 1800; // 1800 30 Minuten Zeit die zwischen zwei Überfällen vergehen muss.
};
...
_ret=[[serverTime,0],"TON_fnc_robbankbranch"] spawn life_fnc_MP;
...
_rip = true;
_onWanted = false;
_kassa = 3500000 + round(random 1500000);
_shop removeAction _action;
_chance = random(100);
if (_chance >= 10 && _chance < 70) then { [[1,format["ALARM! - Bankfiliale: %1 wird ausgeraubt!", _shop]],"life_fnc_broadcast",west,false] spawn life_fnc_MP; };
if(_chance >= 70) then { 
	...
	[[1,format["ALARM! - Bankfiliale: %1 wird ausgeraubt!", _shop]],"life_fnc_broadcast",west,false] spawn life_fnc_MP; 
};
...
if(_rip) then
{
	life_nextrobbankbranch = servertime + _robdelay;
	publicVariable "life_nextrobbankbranch";
	while{true} do
	{
		sleep 6;
		_cP = _cP + 0.01;
		...
		if (_chance >= 70) then {
			_marker = createMarker ["Marker201", _Pos]; //by ehno: Place a Maker on the map
			"Marker201" setMarkerColor "ColorRed";
			"Marker201" setMarkerText "!Achtung! Überfall !Achtung!";
		...
		if!(alive _robber) exitWith {deleteMarker "Marker201"; _rip = false; 5 cutText ["","PLAIN"];};
		if(life_istazed) exitWith {deleteMarker "Marker201"; _rip = false; 5 cutText ["","PLAIN"];};
		if(player getVariable ["Re-strained",FALSE]) exitWith { deleteMarker "Marker201"; _rip = false; 5 cutText ["","PLAIN"];};
		if (currentWeapon _robber isEqualTo "") exitWith { 
			...
			deleteMarker "Marker201"; _rip = false; 5 cutText ["","PLAIN"]; 
		};
	};
	if!(alive _robber) exitWith { _rip = false; deleteMarker "Marker201"; 5 cutText ["","PLAIN"];};
	if(life_istazed) exitWith {deleteMarker "Marker201"; _rip = false; 5 cutText ["","PLAIN"];};
	if(player getVariable ["Re-strained",FALSE]) exitWith { deleteMarker "Marker201"; _rip = false; 5 cutText ["","PLAIN"];};
	if (currentWeapon _robber isEqualTo "") exitWith {deleteMarker "Marker201"; _rip = false; 5 cutText ["","PLAIN"]; };
	if(_robber distance _shop > 10.5) exitWith { deleteMarker "Marker201"; 5 cutText ["","PLAIN"]; _rip = false; };
	5 cutText ["","PLAIN"];
	...
	deleteMarker "Marker201"; // by ehno delete maker
	CASH = CASH + _kassa;
	_rip = false;
	_run = [] spawn {
		life_use_atm = false;
...
_action = _shop addAction["Bankfiliale Ausrauben",life_fnc_robBankBranch,'license_civ_rebel'];