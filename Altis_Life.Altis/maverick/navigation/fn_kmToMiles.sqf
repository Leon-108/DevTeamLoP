...
if (_convert) then {
	...
	format ["%1 %2", _ret, [localize "STR_navigationEinheit_Miles", "mi."] select _short];
} else {
	if (_distance >= 1000) then {
		format ["%1 %2", (_distance / 1000) toFixed 1, [localize "STR_navigationEinheit_Kilometer", "km"] select _short]
	} else {
		private _shortTerm = [localize "STR_navigationEinheit_Meter", "m"] select _short;
