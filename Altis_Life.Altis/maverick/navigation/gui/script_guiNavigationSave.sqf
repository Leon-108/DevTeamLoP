...
switch _mode do {
	...
	case "saveRoute": {
		...
		if (_text isEqualTo "") exitWith {
			[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgNoRouteNameGiven")),"RED",10] spawn life_fnc_notification_system;
		};

		if (count _text > 30) exitWith {
			[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgRouteNameLimit")),"RED",10] spawn life_fnc_notification_system;
		};
		...
		if ({!(_x in _allowed)} count toArray _text > 0) exitWith {
			[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgIllegalChars")),"RED",10] spawn life_fnc_notification_system;
		};
		...
		if (_savedLocation isEqualTo -1) exitWith {
			[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgRouteLimit")),"RED",10] spawn life_fnc_notification_system;
		};
		...
		closeDialog 0;
		[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgRouteSaved")),"GREEN",5] spawn life_fnc_notification_system;
	};
