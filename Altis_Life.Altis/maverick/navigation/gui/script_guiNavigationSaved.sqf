...
switch _mode do {
	...
	case "start": {
		if !(isNil "MAV_gps_v_currentRoute") exitWith {
			[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgFinishRouteFirst")),"RED",10] spawn life_fnc_notification_system;
		};
	...
	case "delete": {
		...
		if !(_curSel isEqualTo -1) then {
			...
			[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgRouteDeleted")),"YELLOW",5] spawn life_fnc_notification_system;
