...
switch _mode do {
	...
	case "commonLocations": {
		...
		if (_roads isEqualTo []) exitWith {[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgNoRoadsFound")),"RED",10] spawn life_fnc_notification_system;};
		...
		if (isNull _road) exitWith {[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgInvalidRoadSegment")),"RED",10] spawn life_fnc_notification_system;};
		if ((roadsConnectedTo _road) isEqualTo []) exitWith {[(getText(missionConfigFile >> "maverick_navigation_cfg" >> "localization" >> "msgInvalidRoadSegment")),"RED",10] spawn life_fnc_notification_system;};
